...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(self, image_features, text_features, attention_mask=None):
        ...
        image_heads = image_heads.permute(0, 2, 1, 3)
        text_heads = text_heads.permute(0, 2, 1, 3)
        ...
        for head_idx in range(self.num_heads):
            curr_image = image_heads[:, head_idx]
            curr_text = text_heads[:, head_idx]
            ...
            img2text_output, _ = self.image_to_text_heads[head_idx](
                query=curr_image_t, key=curr_text_t, value=curr_text_t,
                key_padding_mask=key_padding_mask
            )
            text2img_output, _ = self.text_to_image_heads[head_idx](
                query=curr_text_t, key=curr_image_t, value=curr_image_t,
                key_padding_mask=key_padding_mask
            )
            img2text = curr_image + img2text_output.transpose(0, 1)
            text2img = curr_text + text2img_output.transpose(0, 1)
...
class SteinKernel(nn.Module):
    ...
    def score_kernel(self, x, score_x, y, score_y):
        ...
        term2 = torch.sum(diff * score_y.unsqueeze(0), dim=-1)
...
class Fluid_MMRec(SequentialRecommender):
    def __init__(self, config, dataset, image_feat, text_feat):
        ...
        # ── 변경: numpy 룩업 테이블로 저장 (인덱싱용) ──────────────────────
        # image_feat: (max_item_id+1, image_dim) numpy float32
        # text_feat : (max_item_id+1, text_dim)  numpy float32
        self.image_feat_np = image_feat
        self.text_feat_np  = text_feat
    ...
    def get_multimodal_features(self, item_seq):
        # ── 변경: RecBole id2token 제거, numpy 룩업 테이블 직접 인덱싱 ──────
        item_np = item_seq.cpu().numpy()          # (B, L)
        mask    = (item_np != 0)                  # (B, L) bool

        image_features = torch.FloatTensor(
            self.image_feat_np[item_np]           # (B, L, image_dim)
        ).to(self.device)
        text_features = torch.FloatTensor(
            self.text_feat_np[item_np]            # (B, L, text_dim)
        ).to(self.device)

        attention_mask = torch.BoolTensor(mask).to(self.device)  # (B, L)

        # padding 위치 피처 0으로
        image_features = image_features * attention_mask.unsqueeze(-1)
        text_features  = text_features  * attention_mask.unsqueeze(-1)
